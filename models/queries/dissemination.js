...
//Read
exports.getDisseminations = () =>db.any(`SELECT id, headline, summary, date, added_by, pr_news, url FROM dissemination d ORDER BY id DESC`);
exports.getAvailableDisseminations = () =>db.any(`SELECT id, headline, summary, date, added_by, pr_news, url FROM dissemination d WHERE include_in_report = true ORDER BY id DESC`);
...
exports.getDisseminationsByDate = (initial,final) =>db.any(pgp.as.format(`SELECT id, headline, summary, date, added_by, pr_news, url FROM dissemination d WHERE include_in_report = true AND date >= $1 AND date <= $2 ORDER BY date`,[initial,final]));
