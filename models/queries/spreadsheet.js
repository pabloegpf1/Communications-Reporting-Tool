...
let impactQuery = `i.date as "Date",m.name as "Media Name",m.url as "Media Web", m_t.type as "Type of Media",m.content as "Media Content", m.coverage as "Media Coverage", 
                ...
                i.language as "Language", i.proactivity as "Proactivity", i.uploaded as "Uploaded", d.pr_news as "PR/News", d.summary as "Summary", i.source_url as "Original URL", NOT d.include_in_report as "Other Media Impact"
...
let disseminationQuery = `date as "Date", headline as "Headline", summary as "Summary", lead_paragraph as "Lead Paragraph", pr_news as "PR/NEWS", url as "URL",
                          NOT include_in_report as "Other Media Dissemination" FROM dissemination`;
...
exports.getDisseminations = (initial_date,final_date) =>db.any(pgp.as.format(`SELECT $1:raw WHERE date >= $2 AND date <= $3 ORDER BY date`,[disseminationQuery,initial_date,final_date]));
